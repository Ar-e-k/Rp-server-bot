import csv
import battle_terrain as battle
import discord
from discord.ext import commands as coms
import copy
import main_percentage as main
import time
...
world=[]
with open("info/World_priv.csv") as world_map:
...
for country in world:
    all_country[country[0].upper()]=main.country_init(country[0])
...
'''async def input_army_seq(ctx, value):
    army=["Soldiers: ", "Archers: ", "Cavalery: ", "Artilery: "]
    if value==None:
        await ctx.channel.purge(limit=amount)
        await ctx.send("Invalid input")
        return None
    value, amount=await input_army(ctx, army)
    value='["%s", "%s", "%s", "%s"]'%(value[0], value[1], value[2], value[3])#'''
        ...
        for guild in client.guilds:
            for role in guild.roles:
                ...
                if role.name.upper()==str(checked[1]).upper():
                    break
            #if roles.name==str(checked)
        #print(checked[1])
        #print(all_country.keys())
...
client.run('NzE5ODAyNDM0MDA3OTkwMzU1.XuDDmg.LWRniZCMgVtp7bAZtrsxGidaHio')
